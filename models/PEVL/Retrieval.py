...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, args):
    ...
    for i,(image, text, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        text_input = tokenizer(text, padding='longest', max_length=config.get('max_tokens', 30), return_tensors="pt").to(device)  
...
def evaluation(model, data_loader, tokenizer, device, config, args):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_input = tokenizer(text, padding='max_length', truncation=True, max_length=config.get('max_tokens', 30), return_tensors="pt").to(device) 
...
def main(args, config):
    ...
    pos_token.append('##')
    postoken_dict = {}
    tokenizer = BertTokenizer.from_pretrained('configs_dm/vocab.txt')
    ...
    for epoch in range(0, max_epoch):
        ...
        if utils.is_main_process():  
            ...
            if not args.test_only:
                val_result, val_i2t, val_t2i = itm_eval(score_val_i2t, score_val_t2i, val_loader.dataset.txt2img, val_loader.dataset.img2txt)  
                print(val_result)
            test_result, test_i2t, test_t2i = itm_eval(score_test_i2t, score_test_t2i, test_loader.dataset.txt2img, test_loader.dataset.img2txt)    
            print(test_result)
            ...
            if args.evaluate: 
                ...
                if not args.test_only:
                    np.save(os.path.join(args.output_dir, "score_val_i2t.txt"), score_val_i2t)
                    np.save(os.path.join(args.output_dir, "score_val_t2i.txt"), score_val_t2i)
                    np.save(os.path.join(args.output_dir, "val_i2t.txt"), val_i2t)
                    np.save(os.path.join(args.output_dir, "val_t2i.txt"), val_t2i)
