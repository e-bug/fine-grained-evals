...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        output = model.get_cross_embeds(encoder_output, encoder_att,
                                        ...
                                        text_atts=text_atts[start + i].repeat(config['k_test'], 1, 1)
                                        )[:, 0, :]
        score = model.itm_head(output)[:, 1]
...
def main(args, config):
    ...
    if args.evaluate:
        ...
        if utils.is_main_process():
            val_result, val_i2t, val_t2i = itm_eval(score_val_i2t, score_val_t2i,
                                                    val_loader.dataset.txt2img, val_loader.dataset.img2txt)
            print(val_result)
            test_result, test_i2t, test_t2i = itm_eval(score_test_i2t, score_test_t2i,
